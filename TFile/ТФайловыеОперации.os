...
// 
// Параметры:
//	Путь - Строка - Путь к файлу или каталог
...
//	Ложь - Были ошибки
//
Функция ДобавитьВАрхив(Путь, ИмяАрхива="") Экспорт
...
Функция СкопироватьКаталог(Знач ПолноеИмяКаталогаИсточника, Знач ПутьКаталогаПриемника="", СоздаватьКаталог=Истина) Экспорт
	...
	Если СоздаватьКаталог Тогда
		ИмяКаталогаПриемника = Файл.Имя;
	Иначе
		ИмяКаталогаПриемника = "";
	КонецЕсли;
	
	// Создадим путь к каталогу приемнику
	Файл = Новый Файл(ПутьКаталогаПриемника);
    Если Не Файл.Существует() Тогда
        СоздатьКаталог(ПутьКаталогаПриемника);
    ИначеЕсли Не Файл.ЭтоКаталог() Тогда
		ТекстОшибки = "Не удается создать каталог " + ПутьКаталогаПриемника + ". По данному пути уже существует файл.";
		Возврат Ложь;
    КонецЕсли;
	
	// Создадим каталог приемника
	ИмяКаталогаПриемника = ОбъединитьПути(ПутьКаталогаПриемника,ИмяКаталогаПриемника);
	Файл = Новый Файл(ИмяКаталогаПриемника);
    Если Не Файл.Существует() Тогда
        СоздатьКаталог(ИмяКаталогаПриемника);
    ИначеЕсли Не Файл.ЭтоКаталог() Тогда
        ТекстОшибки = "Не удается создать каталог " + ИмяКаталогаПриемника + ". По данному пути уже существует файл.";
		Возврат Ложь;
    КонецЕсли;	
	...
	Для Каждого Файл Из Файлы Цикл
		ПутьКопирования = ОбъединитьПути(ИмяКаталогаПриемника, Файл.Имя);
