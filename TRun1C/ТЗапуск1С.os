...
//*****************************************************************
// Захватывает объекты в хранилище
//
// Параметры:
//	ПутьИмяФайлаОбъектов - путь к файлу со списком объектов, которые будут участвовать в операции. Если файл указан – в операции участвуют только указанные в файле объекты, в противном случае участвует вся конфигурация
//	ПолучатьОбъектыИзХранилища - получать захваченные объекты, если потребуется.
...
//*******************************************************************
// Помещает измененные объекты в хранилище
//
// Параметры:
// 	ПутьИмяФайлаОбъектов – путь к файлу со списком объектов, которые будут участвовать в операции. Если файл указан – в операции участвуют только указанные в файле объекты, в противном случае участвует вся конфигурация.
//	ТекстКомментария - текст комментария к помещаемым объектам.
//	ОставлятьОбъектыЗахваченными - оставлять помещенные объекты захваченными. Если не указан, то после помещения изменений будет отменен захват для объектов, участвующих в операции.
//	ОчищатьСсылкиНаУдаленныеОбъекты - описывает поведение при обнаружении ссылок на удаленные объекты:
