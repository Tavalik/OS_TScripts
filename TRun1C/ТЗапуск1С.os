...
// 		ConfigurationRepository – конфигурация из хранилища конфигурации;
//		File – файл конфигурации/расширения конфигурации.
//	ИмяФайлаПервойКонфигурации - Строка - Путь к файлу первой конфигурации, указывается если параметр "ТипПервойКонфигурацииДляСравнения" имеет значение "File";
//	ИмяФайлаВторойКонфигурации - Строка - Путь к файлу второй конфигурации, указывается если параметр "ТипВторойКонфигурацииДляСравнения" имеет значение "File";
...
//		Ложь - Были ошибки
//
Функция СравнитьКонфигураций(
	ПутьИмяФайлаОтчета,
	ТипПервойКонфигурации = "MainConfiguration",
	ТипВторойКонфигурации = "DBConfiguration",
	ИмяФайлаПервойКонфигурации = "",
	ИмяФайлаВторойКонфигурации = "",
	...
	// Есть ли подключение к хранилищу
	Если ТипПервойКонфигурации = "MainConfiguration" ИЛИ ТипВторойКонфигурации = "MainConfiguration" Тогда
		ИспользуетсяХранилище = Истина;
	Иначе
		ИспользуетсяХранилище = Ложь;
	КонецЕсли;
	...
	Попытка
		СтрокаЗапуска = 
			...
			?(ТипПервойКонфигурации = "File", ВставитьПараметрЗапуска(ИмяФайлаВторойКонфигурации,"FirstConfigurationKey ","-"),"") +
			ВставитьПараметрЗапуска(ТипВторойКонфигурации,"SecondConfigurationType ","-", Ложь) +
			?(ТипВторойКонфигурации = "File", ВставитьПараметрЗапуска(ИмяФайлаВторойКонфигурации,"SecondConfigurationKey ","-"),"") +			
			?(ПравилоУстановкиСоответствийОбъектов <> "", ВставитьПараметрЗапуска(ПравилоУстановкиСоответствийОбъектов,"MappingRule ","-"),"") + 
...
Функция ЕстьИзмененияВОтчетеОСравненииКраткийТекст(ПутьКФайлуОтчетаОСравнении) Экспорт
	...
	Файл = Новый ЧтениеТекста(ПутьКФайлуОтчетаОСравнении);
	ВесьФайлОднойСтрокой = Файл.Прочитать();			
	Файл.Закрыть();
	
	Если ВесьФайлОднойСтрокой = "
		|	***- Объект изменен
...
Функция ЕстьИзмененияВХранилище() Экспорт
	...
	Если Не СравнитьКонфигураций(ВременныйФайлСравнения,"MainConfiguration","ConfigurationRepository") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ЗахватитьВХранилище(ПутьИмяФайлаОбъектов = "", ПолучатьОбъектыИзХранилища = Истина) Экспорт
	...
	Попытка
		СтрокаЗапуска = 
			...
			ВставитьПараметрЗапуска(ПутьИмяФайлаОбъектов,"Objects ","-") + 
