...
Функция ОбновитьКонфигурациюИзХранилища(НомерВерсии="", ПолучатьЗахваченныеОбъекты = Ложь, Форсировать = Истина, ПутьИмяФайлаОбъектов = "") Экспорт
...
Функция ПолучитьТЗОбъектовИзОтчетаСравнения (ФайлОтчетаСравнения) Экспорт
	...
	//Текст.КоличествоСтрок(),2
	мсСписокОбъектов = Новый массив(Текст.КоличествоСтрок(),2);
	...
	// Если Не мсСписокОбъектов.Количество()>0 Тогда
		// ТекстОшибки = "Нет измененных объектов.";
		// Возврат Неопределено;
	// КонецЕсли;
