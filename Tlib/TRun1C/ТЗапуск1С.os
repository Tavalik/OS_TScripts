...
Функция СравнитьКонфигурации(
	...
	Попытка
		СтрокаЗапуска = 
			...
			ВставитьПараметрЗапуска(ПутьИмяФайлаОтчета,"ReportFile ","-", Ложь) +			
			"/DisableStartupMessages " +
			?(ПараметрыЗапуска.ВидныДействияВПакетномРежиме,"/Visible ","") + 
			?(ЗначениеЗаполнено(ПараметрыЗапуска.ФайлДляЗаписиРезультатовРаботыВПакетномРежиме),ВставитьПараметрЗапуска(ПараметрыЗапуска.ФайлДляЗаписиРезультатовРаботыВПакетномРежиме,"DumpResult "),"");
...
Функция СоздатьФайлСпискаОбъектов(ПолныйПутьКФайлуОтчета="", ПолныйПутьКФайлуСпискаОбъектов="") Экспорт
	...
	ОписаниеСтроки = Новый ОписаниеТипов("Строка",,КС); 
	тзСписокОбъектов = Новый ТаблицаЗначений;	
	тзСписокОбъектов.Колонки.Добавить("ТипИзменения", ОписаниеСтроки);	
	тзСписокОбъектов.Колонки.Добавить("Объект", ОписаниеСтроки);

    ОбъектНеЗахвачен = Ложь;
	Для НомерСтроки = 1 По Текст.КоличествоСтрок() Цикл 
		...
		Если Найти(Стр, "- ***") <> 0 или Найти(Стр, "- <--") или Найти(Стр, "- -->") <> 0 Тогда
			Если Найти(Стр, "- ***") <> 0 Тогда
				ТипИзменения = "Изменено"
			ИначеЕсли Найти(Стр, "- -->") <> 0 Тогда
				ТипИзменения = "ТолькоВПервой";
			ИначеЕсли Найти(Стр, "- <--") Тогда
				ТипИзменения = "ТолькоВоВторой";									
			КонецЕсли;
			Если КоличествоТабов = 2 Тогда												
				НоваяСтрокаТЗ = тзСписокОбъектов.Добавить();									
				Объект = Сред(Стр,КоличествоТабов + 6);					
				НоваяСтрокаТЗ.Объект = Объект;
				НоваяСтрокаТЗ.ТипИзменения = ТипИзменения;					
				ПредОбъект = НоваяСтрокаТЗ.Объект;
				ТипИзмененияПредОбъект = ТипИзменения;
			...
			Если КоличествоТабов = 3 Тогда										
				...
				Если ПредОбъект = ТекОбъект и ОбъектФормаИлиМакет и УровеньПредыдущегоОбъекта = 2 Тогда					
					НоваяСтрокаТЗ.Объект = Сред(Стр,КоличествоТабов + 6);					
					НоваяСтрокаТЗ.ТипИзменения = ТипИзменения;					
					ОбъектНеЗахвачен = Истина;					
				...
				КонецЕсли;
				Если ПредОбъект = ТекОбъект И Не ОбъектФормаИлиМакет И ОбъектНеЗахвачен Тогда
					НоваяСтрокаТЗ = тзСписокОбъектов.Добавить();										
					НоваяСтрокаТЗ.Объект = Объект;					
					НоваяСтрокаТЗ.ТипИзменения = ТипИзменения;					
					УровеньОбъекта = КоличествоТабов; 					
				КонецЕсли;	
				Если ПредОбъект = ТекОбъект и ОбъектНеЗахвачен и ТипИзменения = "ТолькоВоВторой" Тогда
					НоваяСтрокаТЗ = тзСписокОбъектов.Добавить();										
					НоваяСтрокаТЗ.Объект = ПредОбъект;					
					НоваяСтрокаТЗ.ТипИзменения = ТипИзмененияПредОбъект;										
					ОбъектНеЗахвачен = Ложь;
				КонецЕсли;							
			КонецЕсли;			
		КонецЕсли;        
	...
	Если Не тзСписокОбъектов.Количество()>0 Тогда
	...
	// Записываем элемент для каждого объекта из ТЗ
	Для каждого Строка Из тзСписокОбъектов Цикл
		Запись.ЗаписатьНачалоЭлемента("Object");		
		Запись.ЗаписатьАтрибут("fullName", Строка.Объект);		
		Если Найти(Строка.Объект, "Подсистема")<>0 Тогда
			Запись.ЗаписатьАтрибут("includeChildObjects", "true");
		Иначе
			Запись.ЗаписатьАтрибут("includeChildObjects", "false");
		КонецЕсли;
		Запись.ЗаписатьКонецЭлемента();		
...
//******************************************************************
//Создает и возвращает структуру параметров запуска
//
Функция СоздатьСтруктуруПраметровЗапуска () Экспорт

	СтруктураПараметровЗапуска = Новый Структура;
	Для Каждого Параметр Из ПараметрыЗапуска Цикл		
		СтруктураПараметровЗапуска.Вставить(Параметр.Ключ,Параметр.Значение);
	КонецЦикла;
...
//*******************************************************************
//Устанавливает параметры запуска из структуры параметров запуска
//Параметры: - Структура параметров запуска.
//
Процедура УстановитьПараметрыЗапуска (СтруктураПараметровЗапуска) Экспорт 
		
	Для Каждого Параметр Из ПараметрыЗапуска Цикл
		Если ЗначениеЗаполнено(СтруктураПараметровЗапуска[Параметр.Ключ]) Тогда
			ПараметрыЗапуска.Вставить(Параметр.Ключ,СтруктураПараметровЗапуска[Параметр.Ключ]);
		КонецЕсли;
	КонецЦикла;
...
//*******************************************************************
// Создает и возвращает таблицу значений со списком измененных объектов на основании отчета о сравнении.
// Колонки таблицы: 
//	 "Объект" 		- Имя объекта;
//	 "ТипИзменения" - возможные значения: "Изменено" (объект присутствует в обеих конфигурациях), "ТолькоВПервой" (объект присутствует только в первой конфигурации),
// 										  "ТолькоВоВторой" (объект присутствует только во второй конфигурации).
Функция ПолучитьТЗОбъектовИзОтчетаСравнения (ФайлОтчетаСравнения) Экспорт

	Если ФайлОтчетаСравнения = "" Тогда
		ТекстОшибки = "Не верное количество параметров. Не указан путь к файлу отчета о сравнении!";
		Возврат Неопределено;		
	КонецЕсли;
	Текст = Новый ТекстовыйДокумент; 
    Попытка
		Текст.Прочитать(ФайлОтчетаСравнения); 
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		Возврат Неопределено;		
	КонецПопытки;
	...
	Возврат тзСписокОбъектов;	

КонецФункции

//**********************************************************************************************
// Обновляет конфигурацию из файла конфигурации поставщика
//
// Параметры:
// ФайлНовойКонфигурацииПоставщика - имя файла с объединяемой конфигурацией (.cf-файл) или с файлом обновления
//		конфигурации (.cfu-файл).
// ФайлНастроекОбъединения - имя файла с настройками объединения конфигураций. Формат и описание файла настроек
//		объединения см. здесь.
// ФайлДваждыИзмененныеОбъекты - имя файла для сохранения дважды измененных объектов.
// ДобавитьОбъектыСНеразрешимымиСсылками - если в настройках объединения есть объекты, не включенные в список
//		объединяемых и отсутствующие в основной конфигурации, но на которые есть ссылки из объектов, включенных
// 		в список, то такие объекты также помечаются для объединения, и выполняется попытка продолжить объединение.
//		Попытки выполняются, пока не останется объектов со ссылками на не включенные объекты, либо пока не
//		выберется вся конфигурация. Аналогично кнопке Пометить все для объединения в окне с неразрешенными
//		ссылками, только с повторением попыток.
// ОчиститьНеразрешимыеСсылки – ссылки на объекты, не включенные в список объединяемых объектов, очищаются. 
//		Аналогично кнопке Продолжить в окне с неразрешенными ссылками
// ВывестиСписокДваждыИзмененных - вывести список всех дважды измененных свойств в файл для вывода служебных сообщений.
// ПродолжитьПриНаличииПредупреждений – проводить объединение в случае наличия:
...
//
Функция ОбновитьКонфигурациюИзФайлаКонфигурацииПоставщика(
	ФайлНовойКонфигурацииПоставщика,
	ФайлНастроекОбъединения = Неопределено,
	ФайлДваждыИзмененныеОбъекты = Неопределено,
	ДобавитьОбъектыСНеразрешимымиСсылками = Ложь,
	ОчиститьНеразрешимыеСсылки = Ложь,
	ВывестиСписокДваждыИзмененных = Истина,
	ПродолжитьПриНаличииПредупреждений = Истина
	) Экспорт
 
	КодВозврата = 0;
	ТекстОшибки = "";
	ЕстьФайлНастроек = Ложь;
	...
	// Доработаем и проверим параметры запуска
	Файл = Новый Файл(ФайлНовойКонфигурацииПоставщика);
	Если Не Файл.Существует() Тогда
		ТекстОшибки = "Не найден файл по пути: " + ФайлНовойКонфигурацииПоставщика;
		Возврат Ложь;
	КонецЕсли;

	Если ЗначениеЗаполнено(ФайлНастроекОбъединения) Тогда
		ЕстьФайлНастроек = Истина;
		Файл = Новый Файл(ФайлНастроекОбъединения);
		Если Не Файл.Существует() Тогда
			ТекстОшибки = "Не найден файл по пути: " + ФайлНастроекОбъединения;
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;

	Если ЗначениеЗаполнено(ФайлДваждыИзмененныеОбъекты) Тогда
			ВыводитьДваждыИзмененные = Истина;
			// Файл = Новый Файл(ФайлДваждыИзмененныеОбъекты);
			// Если Не Файл.Существует() Тогда
			// 	ТекстОшибки = "Не найден файл по пути: " + ФайлДваждыИзмененныеОбъекты;
			// 	Возврат Ложь;
			// КонецЕсли;
		Иначе
			ВыводитьДваждыИзмененные = Ложь;
	КонецЕсли;
	...
	Попытка
		СтрокаЗапуска = 
			...
			?(ИспользуетсяХранилище,
				...
				ВставитьПараметрЗапуска(ПараметрыЗапуска.ИмяПользователяХранилища,"ConfigurationRepositoryN") +
				ВставитьПараметрЗапуска(ПараметрыЗапуска.ПарольПользователяХранилища,"ConfigurationRepositoryP"),"") +			
			"/UpdateCfg " + ФайлНовойКонфигурацииПоставщика + " " +			
			?(ЕстьФайлНастроек,"-Settings " + ФайлНастроекОбъединения + " ","") + 
			?(ДобавитьОбъектыСНеразрешимымиСсылками, "-IncludeObjectsByUnresolvedRefs ","") +
			?(ОчиститьНеразрешимыеСсылки, "-ClearUnresolvedRefs ","") +
			?(ВывестиСписокДваждыИзмененных, "-DumpListOfTwiceChangedProperties ","") +
			?(ПродолжитьПриНаличииПредупреждений, "-force "," ") +
			?(ВыводитьДваждыИзмененные," /Out " + ФайлДваждыИзмененныеОбъекты + " ","") +			
			"/DisableStartupMessages " +
			?(ПараметрыЗапуска.ВидныДействияВПакетномРежиме,"/Visible ","") + 
			?(ЗначениеЗаполнено(ПараметрыЗапуска.ФайлДляЗаписиРезультатовРаботыВПакетномРежиме),ВставитьПараметрЗапуска(ПараметрыЗапуска.ФайлДляЗаписиРезультатовРаботыВПакетномРежиме,"DumpResult "),"");
...
//	ПутьКФайлам - путь к папке в которой будет создан файл настроек объединения.
//	ИмяКонфигурации - имя конфигурации, как оно указано в свойствах конфигурации.
//
...
//		Путь к файлу списка объектов - Файл создан успешно.
//
Функция СоздатьФайлНастроекОбъединения(ПутьКФайлам = Неопределено, ИмяКонфигурации = Неопределено) Экспорт
	ТекстОшибки = "";
	//Создаем файл списка объектов
	Если НЕ ПутьКФайлам = Неопределено Тогда					
			УказанныйКаталог = Новый Файл(ПутьКФайлам);
			Если Не УказанныйКаталог.Существует() Тогда
				ТекстОшибки = "Указанный путь к файлам не существует!";
				Возврат Неопределено;
			КонецЕсли;
		Иначе
			ТекстОшибки = "Не указана дериктория сохранения файла настроек!";
			Возврат Неопределено;
	КонецЕсли;

	Если ИмяКонфигурации = Неопределено Тогда
		ТекстОшибки = "Не указано имя конфигурации!";
		Возврат Неопределено;		
	КонецЕсли;
	...
	Запись.ЗаписатьНачалоЭлемента("Objects");		
	Запись.ЗаписатьНачалоЭлемента("Configuration");
	Запись.ЗаписатьНачалоЭлемента("MergeRule");
	Запись.ЗаписатьТекст("DoNotMerge");	
	Запись.ЗаписатьКонецЭлемента();		
	Запись.ЗаписатьКонецЭлемента();		
	Запись.ЗаписатьКонецЭлемента();		
	Запись.ЗаписатьКонецЭлемента();			
	Запись.Закрыть(); 	
	ПолныйПутьКФайлуНастроекОбъединения = ПутьКФайлам + "НастройкиОбъединения.xml";
	ФайлНастроек = новый файл(ПолныйПутьКФайлуНастроекОбъединения);
	Если ФайлНастроек.Существует() Тогда
		Попытка
			УдалитьФайлы(ПолныйПутьКФайлуНастроекОбъединения);
		Исключение
			ТекстОшибки = "Файл " + ПолныйПутьКФайлуНастроекОбъединения + " уже существует. Не удалось перезаписать файл.";
			Возврат Неопределено;
		КонецПопытки;
	КонецЕсли;
	ПереместитьФайл(ВременныйФайл, ПолныйПутьКФайлуНастроекОбъединения);
	Возврат ПолныйПутьКФайлуНастроекОбъединения;	
КонецФункции
